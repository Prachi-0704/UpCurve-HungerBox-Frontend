...
// import Header from '../User/Header'; // Import your Header component
...
//         const token = localStorage.getItem('token');
//         try {
//             const response = await axios.delete('http://localhost:8081/api/cart/clear', {
//                 headers: {
//                     'Content-Type': 'application/json',
//                     'Authorization': `Bearer ${token}`
//                 }
...
//                                 {cartItems.length === 0 ? (
//                                     <tr>
//                                         <td colSpan="5" className="no-items">No items in cart</td>
//                                     </tr>
//                                 ) : (
//                                     cartItems.map((item) => (
//                                         <tr key={item.cartItemId}>
//                                             <td><img src={item.food.images[0]} alt={item.food.foodName} className="cart-item-image" /></td>
//                                             <td>{item.food.foodName}</td>
//                                             <td>Rs {item.food.price}</td>
//                                             <td>{item.food.calories}</td>
//                                             <td>Rs {item.totalPrice}</td> {/* Show item total price */}
//                                         </tr>
//                                     ))
//                                 )}
//                             </tbody>
...
//                                         <td>Rs {totalPrice.toFixed(2)}</td> {/* Display total price */}
//                                     </tr>
//                                 </tfoot>
//                             )}
//                         </table>
...
//                         <button onClick={handleBackHome} className="btn btn-primary">Back Home</button>
...
const CartPage = () => {
    ...
    // Calculate the total price for each item and the overall total
    const totalPrice = cartItems.reduce((acc, item) => acc + (item.food.price * item.quantity), 0);
                    ...
                    <div className="box">
                        <table className="cart-table">
                            ...
                            <tbody>
                                {cartItems.length === 0 ? (
                                    <tr>
                                        <td colSpan="6" className="no-items">No items in cart</td>
                                    </tr>
                                ) : (
                                    cartItems.map((item) => (
                                        <tr key={item.cartItemId}>
                                            ...
                                            <td>Rs {(item.food.price * item.quantity).toFixed(2)}</td> {/* Show item total price */}
                                        </tr>
                                    ))
                                )}
                            </tbody>
